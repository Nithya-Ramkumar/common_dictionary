...
class ExtractorOrchestrator:
    ...
    def flatten_entity_results(self, entity_dict, id_val=None, source=None, default_confidence=1.0):
        """
        ...
        # Use provided id, or try to extract from entity_dict
        # Look for any type of ID (sid, pubchem_cid, cid, etc.)
        entity_id = id_val or entity_dict.get("sid") or entity_dict.get("pubchem_cid") or entity_dict.get("cid") or entity_dict.get("id")
    ...
    def run(self):
        ...
        for entity_type, entity_schema in entity_types.items():
            ...
            for search_map in search_mappings:
                ...
                # Collect all values from all filters that have 'values' lists
                all_filter_values = {}
                for filter in filters:
                    attr = filter['attribute']
                    values = filter.get('values', [])
                    if values:
                        if attr not in all_filter_values:
                            all_filter_values[attr] = []
                        all_filter_values[attr].extend(values)
                
                # If we have values to process, call search once with all values
                if all_filter_values:
                    try:
                        search_results = source_instance.search(
                            entity_type=entity_type,
                            filters=filters,  # Pass original filters structure
                            all_values=all_filter_values,  # Pass collected values
                    ...
                    for result in search_results:
                        ...
                        for key, key_map in key_mappings.items():
                            ...
                            for key_source in key_map.get('sources', []):
                                ...
                                # Extract any available ID (sid, pubchem_cid, cid, etc.)
                                entity_id = result.get('sid') or result.get('pubchem_cid') or result.get('cid') or result.get('id')
                                key_flat_records.extend(self.flatten_entity_results(key_result, id_val=entity_id, source=key_source_name))
                        # Flatten search-based result and add to all_flat_results
                        # Extract any available ID (sid, pubchem_cid, cid, etc.)
                        entity_id = result.get('sid') or result.get('pubchem_cid') or result.get('cid') or result.get('id')
                        all_flat_results.extend(self.flatten_entity_results(result, id_val=entity_id, source=source_name))
                        # Add all key-based flat records
                        all_flat_results.extend(key_flat_records)
                else:
                    # If no filters have 'values', use original behavior
                    try:
                        search_results = source_instance.search(
                            entity_type=entity_type,
                            filters=filters,
